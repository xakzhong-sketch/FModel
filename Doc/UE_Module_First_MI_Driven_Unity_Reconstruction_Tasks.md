...
```text
Generated under K:\WorkSpace\trunk\ExportedProject\Assets\Shaders\Subnautica2\MaterialModules.
Runtime/Master/Layer/Blend modules and .module.json metadata are generated by generate_unity_layer_stack_shader.py.
```
...
```text
LayerStack_C43830F3.shader was generated under Assets/Shaders/Subnautica2/Reconstructed/M_LayerStandard.
Generated passes are UniversalGBuffer, ShadowCaster, and DepthOnly.
No UniversalForward, FORWARD_PLUS, or DepthNormalsPass references remain in generated shader/module files.
unity_shader_assignment.json reports reuse_existing when the generated registry is supplied.
Generated GBuffer/Shadow/Depth pass pragmas were aligned with Unity 6 URP Lit.shader.
Unity Editor validation support was added:
  Template: CUE4Parse/CUE4Parse.ShaderBundleExporter/Tools/unity_shader_validation/SN2ShaderValidation.cs.txt
  Project copy: K:\WorkSpace\trunk\ExportedProject\Assets\Editor\SN2ShaderValidation.cs
  Menu: Tools/Subnautica2/Validate LayerStack Shader
Batchmode validation was attempted with K:\Unity\6000.0.62f1\Editor\Unity.exe.
The first direct attempt against ExportedProject did not run because the project was open in another Unity instance.
An isolated Unity 6000.0.62f1 validation project was created under D:\Tmp\SN2ShaderValidationProject to avoid closing the user's open Editor.
After fixing generated ShaderLab headers, validation passed:
  Report: D:\Tmp\ShaderBundles\MI_CG_RockSmooth_01a.layeraware_test.bundle\unity_shader_validation_report_after_header_fix.json
  Log: D:\Tmp\ShaderBundles\MI_CG_RockSmooth_01a.layeraware_test.bundle\unity_shader_validation_editor_after_header_fix.log
  Result: Ok=true, ShaderIsSupported=True, ErrorCount=0, WarningCount=0.
```
...
python D:\Github\FModel\CUE4Parse\CUE4Parse.ShaderBundleExporter\Tools\unity_material_apply_ue_params.py `
  ...
  --layer-aware `
  --assign-shader-meta "K:\...\LayerStack_C43830F3.shader.meta" `
...
```text
The real Unity material was updated with --apply, --add-missing, --layer-aware, and --assign-shader-meta.
Material:
  K:\WorkSpace\trunk\ExportedProject\Assets\Art\Environment\Biome\CoralGarden\Rocks\Material\MI_CG_RockSmooth_01a.mat
Backup:
  K:\WorkSpace\trunk\ExportedProject\Assets\Art\Environment\Biome\CoralGarden\Rocks\Material\MI_CG_RockSmooth_01a.mat.bak_20260608_182914
Apply report:
  D:\Tmp\ShaderBundles\MI_CG_RockSmooth_01a.layeraware_test.bundle\unity_material_restore_real_apply.json
Second dry-run report:
  D:\Tmp\ShaderBundles\MI_CG_RockSmooth_01a.layeraware_test.bundle\unity_material_restore_real_second_dryrun.json
Second dry-run result:
  Matched=58
  MatchedByStableKey=36
  MatchedByLegacyNameFallback=22
  Added=0
  Collisions=0
  ShaderChanged=false
  MissingTextureGuids=3
Missing Unity assets:
  T_RockLichenMasks_01a
  Curve_Base_Atlas
Missing GUIDs are documented and remain null placeholders until those Unity assets are imported.
```
...
```text
/Game/Art/Environment/Biome/CoralGarden/Rocks/Material/MI_CG_RockSmooth_01d
```

Evidence:

```text
D:\Tmp\ShaderUsageTest\analysis\material_family_reconstruction_priority.json groups it with the same Layer/Blend module sequence as MI_CG_RockSmooth_01a.
```
...
```text
Exported D:\Tmp\ShaderBundles\MI_CG_RockSmooth_01d.layeraware_test.bundle.
Verify: OK.
Assignment: extend_existing.
AssignedUnityShader: Subnautica2/Reconstructed/M_LayerStandard/LayerStack_C43830F3.
Reason: Parent/modules/features are compatible, but exact hashes differ.
Diff source:
  ModuleBranchHash is identical.
  PropertySchemaHash differs.
  StableKey-only differences:
    First-only: LayerParameter:0:Normal Intensity
    First-only: LayerParameter:1:Specular Multiplier
    Second-only: LayerParameter:0:Specular Multiplier
No MissingBranches were reported.
```
...
```text
No existing Unity .mat for MI_CG_RockSmooth_01d was found, so a new .mat was created.
Material:
  K:\WorkSpace\trunk\ExportedProject\Assets\Art\Environment\Biome\CoralGarden\Rocks\Material\MI_CG_RockSmooth_01d.mat
Command used --create-if-missing, --add-missing, --layer-aware, and --assign-shader-meta.
Assigned shader:
  Subnautica2/Reconstructed/M_LayerStandard/LayerStack_C43830F3
Apply report:
  D:\Tmp\ShaderBundles\MI_CG_RockSmooth_01d.layeraware_test.bundle\unity_material_restore_create_apply.json
Second dry-run report:
  D:\Tmp\ShaderBundles\MI_CG_RockSmooth_01d.layeraware_test.bundle\unity_material_restore_second_dryrun.json
Second dry-run result:
  Matched=57
  MatchedByStableKey=57
  MatchedByLegacyNameFallback=0
  Added=0
  ShaderChanged=false
  MissingTextureGuids=3
Shader registry now records both validated MIs under LayerStack_C43830F3.
```
...
python -m py_compile CUE4Parse\CUE4Parse.ShaderBundleExporter\Tools\generate_unity_layer_stack_shader.py
```
